...
class DelayPredictorDummy:
    ...
    def predict(self, station_ids, timestamps):
        ...
        input_df = spark.createDataFrame(zip(station_ids, timestamps), ["station_id", "timestamp"])
    
        # Extract hour and minute from input_df.timestamp
        input_df = input_df.withColumn('timestamp_hour', hour(col('timestamp')))
        input_df = input_df.withColumn('timestamp_minute', minute(col('timestamp')))
